...
import { useState } from 'react'; // O useState já estava aqui, só confirmando
...
function InterestCalculator() {
    ...
    // --- NOVO ---
    // 1. Estado para controlar o feedback do botão "copiar"
    ...
    // --- NOVO ---
    // 2. Função para lidar com o clique do botão de copiar
    const handleCopy = () => {
        navigator.clipboard.writeText(projectedBalance.toFixed(8));
        setCopied(true);
        // Depois de 2 segundos, o texto volta para "Copiar"
        setTimeout(() => {
            setCopied(false);
        }, 2000);
    };
    ...
    return (
        <div className="bg-gray-800 p-8 rounded-xl shadow-2xl w-full max-w-2xl">
            ...
            {/* Inputs do Usuário (sem alteração) */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                {/* ... código dos inputs continua o mesmo ... */}
                <div>
                    <label htmlFor="initialBalance" className="block text-sm font-medium text-gray-300 mb-2">Saldo Inicial (BTC)</label>
                    <input type="number" step="0.00000001" id="initialBalance" value={initialBalance} onChange={(e) => setInitialBalance(parseFloat(e.target.value))} className="w-full bg-gray-700 border border-gray-600 rounded-lg p-3 focus:ring-2 focus:ring-cyan-500 focus:outline-none" />
                </div>
                <div>
                    <label htmlFor="dailyEarnings" className="block text-sm font-medium text-gray-300 mb-2">Ganhos Diários (BTC)</label>
                    <input type="number" step="0.00000001" id="dailyEarnings" value={dailyEarnings} onChange={(e) => setDailyEarnings(parseFloat(e.target.value))} className="w-full bg-gray-700 border border-gray-600 rounded-lg p-3 focus:ring-2 focus:ring-cyan-500 focus:outline-none" />
                </div>
                <div>
                    <label htmlFor="years" className="block text-sm font-medium text-gray-300 mb-2">Período (Anos)</label>
                    <input type="number" id="years" value={years} onChange={(e) => setYears(Number(e.target.value))} className="w-full bg-gray-700 border border-gray-600 rounded-lg p-3 focus:ring-2 focus:ring-cyan-500 focus:outline-none" />
                </div>
            </div>
                ...
                <div className="bg-gray-700 p-6 rounded-lg text-center">
                    {initialBalance < 0.0003 ? (
                        ...
                        <>
                            ...
                            {/* --- NOVO --- */}
                            {/* 3. Botão de Copiar adicionado aqui */}
                            <button
                                onClick={handleCopy}
                                className={`px-4 py-2 text-sm rounded-md transition-colors duration-300 ${copied ? 'bg-green-500 text-white' : 'bg-cyan-600 hover:bg-cyan-500 text-white'}`}
                            >
